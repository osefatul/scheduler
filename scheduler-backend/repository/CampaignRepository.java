...
public interface CampaignRepository extends JpaRepository<CampaignMapping, String> {
        ...
        @Query(value = "SELECT c.* FROM [dbo].[campaigns] c "
                        ...
                        + "ORDER BY c.created_date ASC, c.id ASC", // CRITICAL FIX: Always order by creation date first
                        nativeQuery = true)
        ...
        @Query(value = "SELECT c.* FROM [dbo].[campaigns] c "
                        ...
                        + "AND m.company_id = :company_id "
                        + "AND (c.status = 'ACTIVE' OR c.status = 'SCHEDULED' OR c.status = 'INPROGRESS') "
                        + "ORDER BY c.created_date ASC, c.id ASC", // CRITICAL FIX: Consistent ordering
                        nativeQuery = true)
