...
export const RMUsersUnEnrollTable = ({
  ...
  const { data: userData } = useGetCampaignUnEnrollUsersByIdQuery(campaignId, {
  ...
  const columns = useMemo(() => [
    ...
    {
      header: {
        <div className="header" onClick={() => handleSort("reason")}>
          Reason <USBIconSort />
        </div>
      },
  ...
  // Optional: Add a useEffect to log changes to enroll
  useEffect(() => {
  }, [enroll, rowSelection]);
      ...
      {isData && test.length === 0 ? (
        ...
        <>
          ...
          <RMtabFooter
            ...
            setShowNotification={(b) => {
              console.log("Notification visibility toggled");
              setShowNotification(b);
            }}
          />
        </>
...
export default RMUsersUnEnrollTable;